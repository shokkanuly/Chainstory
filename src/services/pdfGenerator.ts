...
export function generatePdfTaxReport(report: FifoAccountingReport): void {
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Retold Tax Report — ${report.walletAddress.slice(0, 8)}</title>      <style>
        @media print {
          body { font-size: 12pt; }
    ...
    <body>
      ...
      <div style="background: #fffbe6; border: 1px solid #ffe58f; padding: 10px 14px; border-radius: 6px; font-size: 11px; color: #873800; margin-bottom: 20px; line-height: 1.4;">
        <strong>Disclaimer:</strong> This report is a <strong>DRAFT estimate</strong> generated by Retold for informational purposes to review with a qualified CPA or tax professional. It is not an official tax filing or formal tax advice.
      </div>
      ...
      <div class="footer">
        <div>Generated by Retold (FIFO cost basis)</div>
        <div>Page 1 of 1</div>
      </div>
